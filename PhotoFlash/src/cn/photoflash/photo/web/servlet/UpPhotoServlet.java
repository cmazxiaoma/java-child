...
public class UpPhotoServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 获取照片的名称
		String photoname = fileItemList.get(0).getString("UTF-8");
		photo.setPhotoname(photoname);

		// 获取照片的描述
		String p_description = fileItemList.get(1).getString("UTF-8");
		photo.setP_description(p_description);

		// 获取相册的id
		String aid = fileItemList.get(2).getString("UTF-8");
		...
		response.sendRedirect(request.getContextPath() + "/user/photo/CreateSuccess.jsp");
	}
