...
public class UpdateAlbumServlet extends HttpServlet {
	 ...
	 * 
	 */
	private static final long serialVersionUID = -939114346692732721L;
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 获取表单传来的id
		String aid = fileItemList.get(0).getString("UTF-8");
		album.setAid(Integer.parseInt(aid));
		// 获取表单页面的相册名
		String albumname = fileItemList.get(1).getString("UTF-8");
		album.setAlbumname(albumname);
		// 获取表单页面的相册简介
		String a_description = fileItemList.get(2).getString("UTF-8");
		album.setA_description(a_description);
		// 获取表单页面的类型
		String type = fileItemList.get(3).getString("UTF-8");
		album.setType(type);
		// 获取相册的权限
		String power = fileItemList.get(4).getString("UTF-8");
		...
		// 重定向到另一个Servlet重新查询Album（相册）
		response.sendRedirect(request.getContextPath() + "/AlbumServlet?method=findAll");
	}
